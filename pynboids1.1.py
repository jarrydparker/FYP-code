# -*- coding: utf-8 -*-
"""
Created on Thu May 26 23:14:21 2022
...
max_time = 5            # max run time in seconds
...
class Boid(pg.sprite.Sprite):
    ...
    def update(self, dt, speed, ejWrap=False):
        ...
        ncount = len(neiboids) #changed walrus set to equality for compatability with python 6 on monarch
        if ( len(neiboids)) > 1:
...
class BoidGrid():  # tracks boids in spatial partition grid
    ...
    def getnear(self, boid, key):
        if key in self.dict:
            ...
            nearby.remove(boid)
        return nearby
    # if none nearby for x = -1, y=-1,0,1, = left edge of flock
    # if none nearby for x = 1, y =-1,0,1, = right edge of flock
    # if none nearby for y = 1, x =-1,0,1, = front of flock
...
# Collects position and velocity data
        # xboids = 
        # vboids = 
# Stops the code after max_time defined at the start of code    
