# -*- coding: utf-8 -*-
"""
Created on Sat Aug 27 17:28:54 2022
...
width = 1500
height = 1500
boid_n = 100 #number of boids (500)
snapshot = 2 #how many snapshots do we use to calculate the interaction parameters (250)
time_steps = 2 # (2000)
...
def update():
    ...
    norm_vel = []
    pos = []
    # xpos = []                                                               # Changes to Nikki's code
    # ypos = []                                                               # Changes to Nikki's code
    for boid in flock:
        boid.edges()
        ...
        # xpos.append(boid.position[0])                                       # Changes to Nikki's code
        # ypos.append(boid.position[1])                                       # Changes to Nikki's code
...
def run(time = time_steps, n_c = n_size): 
    ...
    # counter = 0
    def simupdate(data):
        points.set_ydata(data[0])
        points.set_xdata(data[1])
        return points,

    def simgenerate_points():
    ...
    for t in range(time): 
        ...
        ani = animation.FuncAnimation(fig, simupdate, simgenerate_points, interval=300)
        # counter=counter+1
...
# may ruin the first time step of Nikki's data
xpos1, ypos1 = updatexy()
fig, ax = plt.subplots()
points, = plt.plot(xpos1,ypos1, '.')
plt.title('Number of Boids = 100')
ax.set_ylim(0, width)
